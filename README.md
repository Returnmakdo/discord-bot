...
- 🖱️ **인터랙티브 UI**: 코어 종류 버튼 선택 후 레벨 입력
- 📦 **5종 코어 지원**: 스킬 / 강화 / 마스터리 / 공용 / 3rd 공용
- ☀️ **재화 계산**: 현재 → 목표 레벨까지 필요한 솔 에르다 & 조각 수량 자동 계산
...
maple-discord-bot/
├── src/
│   ├── index.js              # 메인 봇 로직 (슬래시 명령어 포함)
│   ├── services/
│   │   ├── crawler.js        # 메이플 크롤러
│   │   ├── discord.js        # Discord 메시지 발송
│   │   ├── nexonApi.js       # 넥슨 Open API 연동
│   │   ├── skillCalculator.js# 6차 스킬 강화 비용 계산
...
- 🧹 `src/index.js` 675줄 → 9줄로 슬림화 (엔트리포인트만 남김)
- 📦 봇 라이프사이클을 `src/bot.js`로 분리
- 📂 슬래시 명령어를 `src/commands/` 로 분리 (exp, skill, 라우터)
- 🔧 순수 함수 유틸 분리: `utils/format.js`, `utils/chart.js`, `utils/expStats.js`
- ✨ 기능 변경 없음 (내부 구조 리팩토링만)

### v1.7.4 (2026.04.21)

- 🌐 슬래시 명령어를 다시 글로벌 등록으로 전환 (모든 서버에서 사용 가능)
- 🧹 `GUILD_ID` 설정 시 과거 길드 등록본을 비워 중복 방지

### v1.7.3 (2026.04.21)

- 🐛 discord.js v14.22+에서 `ready` 이벤트가 `clientReady`로 변경된 것에 대응 (슬래시 명령어 등록이 호출되지 않던 문제)

### v1.7.2 (2026.04.21)

- 🐛 `GUILD_ID` 지정 시 글로벌 명령어가 함께 노출돼 **중복**으로 보이던 문제 수정
- 🧹 `GUILD_ID` 있으면 글로벌 명령어를 비우고 길드에만 등록

### v1.7.1 (2026.04.21)

- ⚡ `GUILD_ID` 지정 시 해당 서버에 슬래시 명령어 즉시 등록 (글로벌 전파 대기 불필요)
- 🧹 미사용 `CHANNEL_ID_ALL` 환경변수 제거

### v1.7.0 (2026.04.21)

- ⚡ `!` 접두 명령어를 슬래시 명령어로 전환 (`/경험치`, `/6차`)
- 🗑️ 음악 재생 기능 및 관련 의존성 전부 제거
- 🔒 `MESSAGE CONTENT INTENT` 의존 제거 (Guilds 인텐트만 사용)
- 🧹 `src/services/music.js` 삭제, `@discordjs/opus`/`@discordjs/voice`/`distube`/`@distube/yt-dlp`/`libsodium-wrappers` 의존성 정리

### v1.6.0 (2026.04.21)

- 🔮 6차 스킬 강화 비용 계산기 추가 (`!6차`)
- 🖱️ 코어 선택 버튼 → 현재/목표 레벨 입력 Modal → 결과 embed
- 📦 5종 코어 지원 (스킬/강화/마스터리/공용/3rd 공용)
- ☀️ 솔 에르다 및 조각 필요량 자동 계산

### v1.4.1 (2026.01.27)

- 🔧 음악 스트림 오류 수정 (play-dl → @distube/ytdl-core)
- ⬆️ Node.js 최소 버전 요구사항 추가 (>=20.18.1)

### v1.4.0 (2026.01.23)

- 🎵 음악 재생 기능 추가 (YouTube)
- 🔍 YouTube 검색/URL 재생, 대기열 관리
- 🔊 볼륨 조절, 일시정지/재개, 현재곡 정보
- 📋 대기열 표시 (최대 10곡 미리보기)
